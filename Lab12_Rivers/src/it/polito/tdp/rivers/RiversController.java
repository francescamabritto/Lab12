...
public class RiversController {
    ...
    void doSelectRiver(ActionEvent event) {
    	try {
    		...
    		model.updateSelectedRiver(river);
    		
    		this.txtStartDate.setText(model.getStartDate(river).toString());
    		this.txtEndDate.setText(model.getEndDate(river).toString());
    		this.txtNumMeasurements.setText(Integer.toString(model.getNumMeasurement(river)));
    		this.txtFMed.setText(model.getFMed(river));
    	}catch(Exception e) {
    		e.printStackTrace();
    		this.txtResult.appendText("Problema: vedi System output.");
    		return;
    	}
    }
